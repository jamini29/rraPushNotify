...
class APNS {
	...
	* @access private
	*/
	private $logPath = 'cli/log/send_and_check_apns_messages.log';

	/**
	* Max files size of log before it is truncated. 1048576 = 1MB. Need to rotate error logs separately
	*
	* @var int
	* @access private
	*/
	...
	* @access private
	*/
	private $certificate = 'certs/cer/ios/com.grinasys.runningforweightlosspro.pem'; // just dummy to skip cert exists error
	...
	* @access private
	*/
	private $sandboxCertificate = 'certs/cer/ios/com.grinasys.walkingfree.pem';  // just dummy to skip cert exists error
	...
	*/
	 ...
	 * Constructor.
	 *
	 * Initializes a database connection and perfoms assigned tasks
	 *
	...
	function __construct($db, $args=NULL, $certificate=NULL, $sandboxCertificate=NULL, $logPath=NULL, $environment=NULL) {
		...
		if(!empty($args)){
            ...
            $args['pushbadge'],
						$args['pushalert'],
						$args['pushsound'],
						isset($args['groupid'])?$args['groupid']:null,
						$environment,
            $args['applistid'],
            $args['profilets']
					);
				...
				case "fetch":
					$this->_fetchMessages(
                                                isset($args['appid'])?$args['appid']:null
                                        );
					break;
	...
	private function checkSetup(){
		...
		if($certificateMod>644)  $this->_triggerError('Production Certificate is insecure! chmod 644.');
		if($sandboxCertificateMod>644)  $this->_triggerError('Sandbox Certificate is insecure! chmod 644.');
	}
	 ...
	 * @param string $customchannel 32 charater unique couchbase channel ???
	 * @param string $pushidentificator 64 character unique device token tied to device id
	 * @param string $pushbadge Whether Badge Pushing is Enabled or Disabled
 	 ...
 	 * @param string $groupid The groupid of the app for message grouping
	 * @access private
	 */
	private function _registerDevice($appid, $customchannel, $pushidentificator, $lang, $gmtoffset, $docid, $pushbadge, $pushalert, $pushsound, $groupid = NULL, $environment, $applistid, $profilets){

		if(strlen($appid)==0) $this->_triggerError('Application Name must not be blank.', E_USER_ERROR);
		else if(strlen($customchannel)>32) $this->_triggerError('Device ID may not be more than 40 characters in length.', E_USER_ERROR);
		else if(strlen($pushidentificator)!=64) $this->_triggerError('Device Token must be 64 characters in length.', E_USER_ERROR);
    else if(strlen($lang)!=2) $this->_triggerError('Lang must be 2 characters in length.', E_USER_ERROR);
                //$gmtoffset
    else if(strlen($docid)==64) $this->_triggerError('DocID must not be blank.', E_USER_ERROR);
		...
		$appid = $this->db->prepare($appid);
		$customchannel = $this->db->prepare($customchannel);
		$pushidentificator = $this->db->prepare($pushidentificator);
		$pushbadge = $this->db->prepare($pushbadge);
		$pushalert = $this->db->prepare($pushalert);
		$pushsound = $this->db->prepare($pushsound);
		$groupid = $this->db->prepare($groupid);
    $lang = $this->db->prepare($lang);
    $gmtoffset = $this->db->prepare($gmtoffset);
    $docid = $this->db->prepare($docid);
    $applistid = $this->db->prepare($applistid);
    $profilets = $this->db->prepare($profilets);
        ...
        (
          ...
          `pushidentificator`=IF(@update := (`profilets` < VALUES(`profilets`)), VALUES(`pushidentificator`), `pushidentificator`),
          `lang`=IF(@update, VALUES(`lang`), `lang`),
          `gmtoffset`=IF(@update, VALUES(`gmtoffset`), `gmtoffset`),
          `docid`=IF(@update, VALUES(`docid`), `docid`),
          `pushbadge`=IF(@update, VALUES(`pushbadge`), `pushbadge`),
          `pushalert`=IF(@update, VALUES(`pushalert`), `pushalert`),
          `pushsound`=IF(@update, VALUES(`pushsound`), `pushsound`),
          `development`=IF(@update, VALUES(`development`), `development`),
          `status`='active',
          `profilets`=IF(@update, VALUES(`profilets`), `profilets`),
          `modified`=IF(@update, NOW(), `modified`);";
		$this->db->query($sql);
	}
	 ...
	 *
	 * called when Apple's Feedback Service responds with an invalid token.
	 *
	 ...
	 * Fetch Messages
	 *
         * @param string $appId to select messages for this appid (different pem keys for app)
	 * @access private
	 */
	private function _fetchMessages($appId){
		// only send one message per user... oldest message first
		$sql = "SELECT
				`apns_messages`.`pid`,
				`apns_messages`.`message`,
				`apns_devices`.`pushidentificator`,
				`apns_devices`.`development`
			FROM `apns_messages`
			LEFT JOIN `apns_devices` ON (`apns_devices`.`pid` = `apns_messages`.`fk_device` AND `apns_devices`.`groupid` = `apns_messages`.`groupid`)
			WHERE `apns_messages`.`status`='queued'
        AND `inprocess`='0'
				AND `apns_messages`.`delivery` <= NOW()
				AND `apns_devices`.`status`='active'".
        (!is_null($appId) ? " AND `apns_devices`.`appid` = '{$this->db->prepare($appId)}'" : "").
	...
	/**
	 * Flush Messages
	 * same as _fetchMessages, but sends all the messages for each device (_fetchMessage sends only the first message for device)
	 *
         * @param string $appId to select messages for this appid (different pem keys for app)
	 * @access private
	 */
	private function _flushMessages($appId){
		// only send one message per user... oldest message first
		$sql = "SELECT
				`apns_messages`.`pid`,
				`apns_messages`.`message`,
    ...
    if($result = $this->db->query($sql)){
          ...
          array_push($rotate, $row);
          $sql1 = "UPDATE `apns_messages`
            SET `inprocess`='1'
            WHERE `pid`='{$this->db->prepare($row['pid'])}'
				    LIMIT 1;";
          $this->db->query($sql1);
        }
      }
...
//						$this->_triggerError("APNS responded with error for pid($identifier). status($status: $desc)", E_USER_NOTICE); //E_USER_ERROR);
...
//				$this->_triggerError("Unregistering Device Token: {$token}.");
        echo "Unregistering Device Token: {$token}.\n";
			}
		}
		fclose($fp);
	}
	 ...
	 * @access public
	 */
	public function newMessage($fk_device=NULL, $delivery=NULL, $groupId=NULL, $campid=NULL){
		...
		if (is_null($fk_device))
		{
			...
			if (!is_null($groupId))
				$sql .= " AND `groupid` = '{$this->db->prepare($groupId)}'";
		...
		$this->message['aps']['groupid'] = $groupId;
		$this->message['send']['to'] = $fk_device;
		$this->message['send']['when'] = $delivery;
    $this->message['send']['campid'] = $campid;
	 ...
	 * @access public
	 */
	public function newMessageByCustomChannel($customChannel=NULL, $delivery=NULL, $groupId=NULL) {
		
		$sql = "SELECT `pid` FROM `apns_devices` WHERE `customchannel`='$customChannel'";

		$result = $this->db->query($sql);
		$row = $result->fetch_array(MYSQLI_ASSOC);

		if ($row != NULL)
			$this->newMessage ($row["pid"], $delivery, $groupId);
	...
	public function queueMessage(){
		...
		$when = $this->message['send']['when'];
                $campId = $this->message['send']['campid'];
		$groupId = is_null($this->message['aps']['groupid']) ? null : $this->db->prepare($this->message['aps']['groupid']);
		$list = (is_array($to)) ? $to : array($to); // some???
		...
		$sql = "
			...
			WHERE `pid` IN (" . implode(', ', $list) . ")
				AND `status`='active'" . (is_null($groupId) ? '' : "
				AND `groupid` = '{$groupId}'");
...
//				}
				...
				if(is_null($usermessage['aps']['groupid'])) {
					unset($usermessage['aps']['groupid']);
				}
				...
				if(empty($usermessage)) {
					...
					$delivery = (!empty($when)) ? "'{$when}'":'unix_timestamp(NOW())';
